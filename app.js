...
app.get('/', async (req, res) => {
  ...
  //Fetch api data
  const data = await (
    await fetch('https://infoboard.mediehuset.net/api/')).json();
...
activity.forEach(elem => {
    ...
    elem.friendly_name = (!elem.friendly_name) ? elem.name : elem.friendly_name
        ...
        // const curtime = (new Date(date.getFullYear(), date.getMonth(), date.getDate()+1, 08, 15, 0).getTime()/1000);
        ...
        if(curhour.length) {
            ...
            temp_list = await data.activity.filter(item => item.stamp >= curhour[0].start && item.stamp <= curhour[0].stop);
        } else {
            // Træk første index ud af fetch data
            const firstkey = data.activity.find(Boolean);
            // Hent data der passer i næstkommende skoledags første skematid (8:15 - 9:20)
            temp_list = await data.activity.filter(item => item.stamp <= (firstkey.stamp+3899));
        }
        ...
        console.log(event.toLocaleDateString('da-DK', options))
  ...
  console.log(weatherData.CurrentData.temperature + weatherData.CurrentData.skyText)
});
